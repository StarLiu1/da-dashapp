...
def update_plots(slider_cutoff, click_data, uTP, uFP, uTN, uFN, pD, data_type, upload_contents, disease_mean, disease_std, healthy_mean, healthy_std, figure):
    ...
    if not ctx.triggered or ctx.triggered[0]['prop_id'] == '.':
        ...
        if trigger_id == 'cutoff-slider' or trigger_id in ['uTP-slider', 'uFP-slider', 'uTN-slider', 'uFN-slider', 'pD-slider', 'disease-mean-slider', 'disease-std-slider', 'healthy-mean-slider', 'healthy-std-slider']:
            H = uTN - uFP
    ...
    if data_type == 'imported':
        ...
        # distribution_fig.add_trace(go.Histogram(x=1-predictions, name='Healthy', opacity=0.75, marker=dict(color='green')))
        ...
        # Generate PDFs for diseased and healthy populations
        ...
        distribution_fig.update_layout(
            title='Diseased vs Healthy Distribution',
            xaxis_title='Value',
            yaxis_title='Count',
            barmode='overlay',
            template='plotly_white',
    ...
    distribution_fig.update_traces(opacity=0.75)

    return roc_fig, cutoff_text, slider_cutoff, optimal_cutoff_text, utility_fig, distribution_fig
